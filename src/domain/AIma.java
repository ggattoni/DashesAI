...
public class AIma {
	...
	public double maxValue(State state, boolean player, double alpha, double beta, int depth) {
		updateMetrics(depth);
	...
	// returns an utility value
	public double minValue(State state, boolean player, double alpha, double beta, int depth) {
		updateMetrics(depth);
		if (state.gameEnded() || depth >= currDepthLimit || timer.timeOutOccurred()) {
			return eval(state, player);
		} else {
	 ...
	 */
	protected double eval(State state, boolean player) {
		if (state.gameEnded()) {
			if (state.getTurn()) {
				return game.getTurn() ? 1.0 : -1.0;
			} else {
				return !game.getTurn() ? 1.0 : -1.0;
			}
		} else {
			heuristicEvaluationUsed = true;
			if (onlyTwoGroupsofTwo(state)) {
				if (state.getTurn()) {
					return game.getTurn() ? -1.0 : 1.0;
				} else {
					return !game.getTurn() ? -1.0 : 1.0;
	...
	/**
	 * Si può prevedere la sconfitta anche nel caso in cui rimangono tanti trattini
	 * singoli e una sequenza di due o più trattini
	 */

	private boolean onlyTwoGroupsofTwo(State s) {
		int groupsCount = 0;

		if (s.isPossible(0, 0, 1)) {
			return false;
		}
		if (s.isPossible(1, 0, 2)) {
			groupsCount++;
		} else if (!s.allLineSigned(1)) {
			return false;
		}
		if ((s.isPossible(2, 0, 2) && !s.isPossible(2, 2, 1)) ||
				(!s.isPossible(2, 0, 1) && s.isPossible(2, 1, 2))) { 
			groupsCount++;
		} else if (!s.allLineSigned(2)) {
			return false;
		}
		if ((s.isPossible(3, 0, 2) && !s.isPossible(3, 2, 1) && !s.isPossible(3, 3, 1)) || 
				(!s.isPossible(3, 0, 1) && s.isPossible(3, 1, 2) && !s.isPossible(3, 3, 1)) || 
				(!s.isPossible(3, 0, 1) && !s.isPossible(3, 1, 1) && s.isPossible(3, 2, 2))) { 
			groupsCount++;
		} else if (!s.allLineSigned(3)) {
			return false;
		}
		if (s.isPossible(4, 0, 2) && !s.isPossible(4, 2, 1) && s.isPossible(4, 3, 2)) {
			groupsCount += 2;
		} else if ((s.isPossible(4, 0, 2) && !s.isPossible(4, 2, 1) && !s.isPossible(4, 3, 1) && !s.isPossible(4, 4, 1))
				|| (s.isPossible(4, 1, 2) && !s.isPossible(4, 0, 1) && !s.isPossible(4, 3, 1) && !s.isPossible(4, 4, 1))
				|| (s.isPossible(4, 2, 2) && !s.isPossible(4, 0, 1) && !s.isPossible(4, 1, 1) && !s.isPossible(4, 4, 1))
				|| (s.isPossible(4, 3, 2) && !s.isPossible(4, 0, 1) && !s.isPossible(4, 1, 1) && !s.isPossible(4, 2, 1))) {
			groupsCount++;
		} else if (!s.allLineSigned(4)) {
			return false;
		}
		if ((s.isPossible(5, 0, 2) && !s.isPossible(5, 2, 1) && s.isPossible(5, 3, 2) && !s.isPossible(5, 5, 1))
				|| (s.isPossible(5, 1, 2) && !s.isPossible(5, 0, 1) && s.isPossible(5, 4, 2) && !s.isPossible(5, 3, 1))
				|| (s.isPossible(5, 0, 2) && !s.isPossible(5, 2, 1) && s.isPossible(5, 4, 2)
						&& !s.isPossible(5, 3, 1))) {
			groupsCount += 2;
		} else if ((s.isPossible(5, 0, 2) && !s.isPossible(5, 2, 1) && !s.isPossible(5, 3, 1) && !s.isPossible(5, 4, 1)
						&& !s.isPossible(5, 5, 1))
				|| (s.isPossible(5, 1, 2) && !s.isPossible(5, 0, 1) && !s.isPossible(5, 3, 1) && !s.isPossible(5, 4, 1)
						&& !s.isPossible(5, 5, 1))
				|| (s.isPossible(5, 2, 2) && !s.isPossible(5, 0, 1) && !s.isPossible(5, 1, 1) && !s.isPossible(5, 4, 1)
						&& !s.isPossible(5, 5, 1))
				|| (s.isPossible(5, 3, 2) && !s.isPossible(5, 0, 1) && !s.isPossible(5, 1, 1) && !s.isPossible(5, 2, 1)
						&& !s.isPossible(5, 5, 1))
				|| (s.isPossible(5, 4, 2) && !s.isPossible(5, 0, 1) && !s.isPossible(5, 1, 1) && !s.isPossible(5, 2, 1)
						&& !s.isPossible(5, 3, 1))) {
			groupsCount++;
		} else if (!s.allLineSigned(5)) {
			return false;
		}
	 ...
	 */
	public List<Action> orderActions(State state, List<Action> actions, boolean player, int depth) {
		return actions;
	}
	...
	public static void main(String[] args) throws InterruptedException, AlreadySignedException {
...
//				long t = System.currentTimeMillis();
				// if (s.getTurn()) {
				AIma ai = new AIma(s, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 5);
				...
				// s = s.result(a);
				// }
//				Thread.sleep(1000 - (t - System.currentTimeMillis()));
//				System.out.println(s.getTurn() ? "Player 1 turn:\n" : "Player 2 turn:\n");
//				System.out.println(s);
//				System.out.println();
			}
			...
			if (s.getTurn())
				...
				player2++;
			System.out.println("" + i + "\tPlayer 1: " + player1 + "\tPlayer 2: " + player2);
		}
	}
